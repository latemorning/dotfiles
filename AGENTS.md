...
- `zsh/`: oh-my-zsh, powerlevel9k, Homebrew shellenv, vi mode, alias, 머신별 local 설정 로딩.
...
- macOS에서 `/Users/KTH/.local/bin/select-input-source`가 있으면 Insert 모드 Escape 이후 Normal 모드 Escape를 한 번 더 눌렀을 때 ABC 입력 소스로 전환합니다.
